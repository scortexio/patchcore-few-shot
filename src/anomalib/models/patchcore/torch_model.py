...
class PatchcoreModel(DynamicBufferModule, nn.Module):
    ...
    def __init__(
        ...
        if self.backbone in MODELS_TIMM and self.backbone in MODELS_TORCH:
            print(f"warning, {self.backbone} found in both TIMM and TORCH, going for TORCH")

        # loading non anti-aliased models from torchvision and timm
        ...
        self.layer_and_embedding_size: dict = {}
    ...
    def forward(self, input_tensor: Tensor) -> Tensor | tuple[Tensor, Tensor]:
        ...
        if not self.layer_and_embedding_size:
            self.layer_and_embedding_size = {
                layer: list(feature.shape[1:][::-1]) for layer, feature in features.items()
            }
            self.layer_and_embedding_size["concatenated"] = list(embedding.shape[1:][::-1])
    ...
    def subsample_embedding(self, embedding: Tensor, sampling_ratio: float) -> None:
        ...
        selected_idxs = sampler.select_coreset_idxs()
        self.memory_bank = embedding[selected_idxs]

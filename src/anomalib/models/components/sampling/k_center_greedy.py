...
class KCenterGreedy:
    ...
    def __init__(self, embedding: Tensor, sampling_ratio: Union[int, float]) -> None:
        self.embedding = embedding

        # sampling ratio is a fraction
        if 0 < sampling_ratio <= 1:
            if sampling_ratio == 1:
                warnings.warn(
                    "Sampling ratio is equal to 1, coreset is applied\
                              but number of points will be the same, this might be\
                              an error"
                )
            self.coreset_size = int(embedding.shape[0] * sampling_ratio)
        else:
            # sampling ratio is an absolute number
            self.coreset_size = int(sampling_ratio)
            assert sampling_ratio > 1  # make sure it is not a negative number
            assert self.embedding.size(0) >= self.coreset_size
    ...
    def select_coreset_idxs(self, selected_idxs: list[int] | None = None) -> list[int]:
        ...
        for _ in tqdm(range(self.coreset_size), total=self.coreset_size):

...
def update_config_with_backbone_config(config: OmegaConf, backbone_config: OmegaConf, backbone: str) -> OmegaConf:
    """Update the experiment configuration with the backbone configuration

    Fields to be updated in config:
        dataset.image_size
        model.backbone
        model.layers
    ...
    Returns:
        the updated config
    """
    config.dataset.image_size = backbone_config[backbone].image_size
    config.model.backbone = backbone
    config.model.layers = backbone_config[backbone].layers
    return config
    ...
    **kwargs,
    ...
    if backbone is not None:
        if backbone_config_path is None:
            raise ValueError("backbone is provided while backbone_config_path is None")
    ...
    if backbone is not None:
        # update config with backbone_config
        config = update_config_with_backbone_config(
            config=config,
            backbone_config=backbone_config,
            backbone=backbone,
        )
    config = update_config_with_experiment_setting(config, **kwargs)
    ...
    if config.project.unique_dir:
        ...
        project_path = project_path / "best_model"
        warn(
